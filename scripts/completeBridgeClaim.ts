 ...
 * Run with: npx ts-node scripts/completeBridgeClaim.ts
 */
...
async function completeBridgeClaim(sourceTxHash: string, sourceDomain: number) {
  ...
  const messageTransmitter = new ethers.Contract(
    SEPOLIA_CONFIG.messageTransmitter,
    MESSAGE_TRANSMITTER_ABI,
    wallet
  );

  const tx = await messageTransmitter.receiveMessage(msg.message, msg.attestation, {
    gasLimit: 500000n,
  });
