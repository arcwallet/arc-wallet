/**
 * Unlock Wallet Component - STRICTLY PASSKEY ONLY
...
const UnlockWalletPasskey: React.FC<UnlockWalletProps> = ({ onUnlock, onReset }) => {
    ...
    const handleUnlock = async () => {
        ...
        try {
            console.log('[UnlockWalletPasskey] Unlocking with biometric...');
            await unlockWallet();
            console.log('[UnlockWalletPasskey] Unlock successful');
            onUnlock();
        } catch (err: any) {
            console.error('[UnlockWalletPasskey] Unlock failed:', err);
    ...
    const handleReset = async () => {
        if (window.confirm('Are you sure you want to reset your wallet? This will delete your encrypted wallet permanently. This action cannot be undone.')) {
            try {
                ...
                console.error('[UnlockWalletPasskey] Reset failed:', error);
            ...
            <div className="relative z-20 w-full max-w-md px-4 animate-in fade-in zoom-in duration-700">
                <div className="space-y-8">
                    ...
                    <div className="text-center space-y-4">
                        <div className="w-24 h-24 mx-auto rounded-full bg-blue-500/10 border border-blue-400/30 flex items-center justify-center backdrop-blur-sm relative">
                            <svg className="w-12 h-12 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                            </svg>
                            {/* Passkey Indicator Badge */}
                            <div className="absolute -bottom-2 -right-2 bg-green-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full border border-slate-900 shadow-lg">
                                PASSKEY
                            </div>
                        </div>
...
export default UnlockWalletPasskey;
