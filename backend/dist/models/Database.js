...
export class Database {
    ...
    async createUser(user) {
        ...
        const now = new Date();
        await run(`INSERT INTO users (id, username, display_name, wallet_address, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?)`, [user.id, user.username, user.displayName, user.walletAddress, now.toISOString(), now.toISOString()]);
    ...
    async updateEncryptedWallet(userId, encryptedWallet) {
        await this.waitForReady();
        const run = promisify(this.db.run.bind(this.db));
        await run('UPDATE users SET encrypted_wallet = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [encryptedWallet, userId]);
    }
    ...
    // Passkey operations
    async createPasskeyCredential(credential) {
        await this.waitForReady();
        const run = promisify(this.db.run.bind(this.db));
        const get = promisify(this.db.get.bind(this.db));
        const now = new Date();
        await run(`INSERT INTO passkey_credentials
    ...
    async getPasskeysByUserId(userId) {
        ...
        return credentials.map((c) => this.mapPasskeyCredential(c));
    }
    ...
    async createSessionKey(sessionKey) {
        ...
        const now = new Date();
        await run(`INSERT INTO session_keys (id, user_id, private_key, private_key_iv, address, expires_at, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [
            sessionKey.id,
            sessionKey.userId,
            encrypted,
    ...
    async getActiveSessionKeysByUserId(userId) {
        await this.waitForReady();
        const all = promisify(this.db.all.bind(this.db));
        const sessionKeys = await all('SELECT * FROM session_keys WHERE user_id = ? AND expires_at > ? ORDER BY created_at DESC', [userId, new Date().toISOString()]);
        return sessionKeys.map((k) => this.mapSessionKey(k));
    }
    async getActiveSessionKeyByAddress(address) {
        await this.waitForReady();
        const get = promisify(this.db.get.bind(this.db));
        const sessionKey = await get('SELECT * FROM session_keys WHERE LOWER(address) = LOWER(?) AND expires_at > ? ORDER BY created_at DESC LIMIT 1', [address, new Date().toISOString()]);
        return sessionKey ? this.mapSessionKey(sessionKey) : null;
    }
    async getSessionKeyById(id) {
        await this.waitForReady();
        const get = promisify(this.db.get.bind(this.db));
        const sessionKey = await get('SELECT * FROM session_keys WHERE id = ?', [id]);
        return sessionKey ? this.mapSessionKey(sessionKey) : null;
    }
    async revokeSessionKey(id) {
        await this.waitForReady();
        const run = promisify(this.db.run.bind(this.db));
    ...
    async createChallenge(challenge) {
        ...
        const get = promisify(this.db.get.bind(this.db));
        const now = new Date();
        await run(`INSERT INTO webauthn_challenges (id, challenge, user_id, type, expires_at, created_at)
       VALUES (?, ?, ?, ?, ?, ?)`, [
            challenge.id,
            challenge.challenge,
            challenge.userId,
    ...
    async getLatestChallengeByType(type) {
        await this.waitForReady();
        const get = promisify(this.db.get.bind(this.db));
        const row = await get('SELECT * FROM webauthn_challenges WHERE type = ? AND expires_at > ? ORDER BY created_at DESC LIMIT 1', [type, new Date().toISOString()]);
        return row ? this.mapChallenge(row) : null;
    }
    async getChallengeByValue(challengeValue, type) {
        await this.waitForReady();
        const get = promisify(this.db.get.bind(this.db));
        const params = [challengeValue, new Date().toISOString()];
    ...
    async getActiveSessionKeyByAddressAndPrivateKey(address, privateKey) {
        await this.waitForReady();
        const get = promisify(this.db.get.bind(this.db));
        const row = await get('SELECT * FROM session_keys WHERE address = ? AND private_key = ? AND expires_at > ? LIMIT 1', [address, privateKey, new Date().toISOString()]);
    ...
    async createBridgeTransaction(transaction) {
        ...
        const lastID = await new Promise((resolve, reject) => {
            this.db.run(`INSERT INTO bridge_transactions
    ...
    async updateBridgeTransaction(id, updates) {
        ...
        values.push(new Date().toISOString());
        values.push(id);
        await run(`UPDATE bridge_transactions SET ${fields.join(', ')} WHERE id = ?`, values);
        return this.getBridgeTransaction(id);
    }
    async getBridgeHistory(userId, limit = 50, offset = 0) {
        await this.waitForReady();
        const all = promisify(this.db.all.bind(this.db));
        const transactions = await all('SELECT * FROM bridge_transactions WHERE user_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?', [userId, limit, offset]);
        return transactions.map(this.mapBridgeTransaction);
    }
    mapBridgeTransaction(row) {
        return {
            id: row.id,
            userId: row.user_id,
    ...
    async createRecoveryToken(data) {
        await this.waitForReady();
        const run = promisify(this.db.run.bind(this.db));
        await run('INSERT INTO recovery_tokens (id, email, token, expires_at) VALUES (?, ?, ?, ?)', [data.id, data.email.toLowerCase(), data.token, data.expiresAt.toISOString()]);
    }
    async getRecoveryToken(token) {
        await this.waitForReady();
        const get = promisify(this.db.get.bind(this.db));
        const row = await get('SELECT * FROM recovery_tokens WHERE token = ? AND used = 0 AND expires_at > ?', [token, new Date().toISOString()]);
        if (!row)
            return null;
        return {
            id: row.id,
    ...
    async deletePasskeysByEmail(email) {
        await this.waitForReady();
        const run = promisify(this.db.run.bind(this.db));
        ...
        const result = await new Promise((resolve, reject) => {
            this.db.run('DELETE FROM passkey_credentials WHERE user_id = ?', [user.id], function (err) {
                if (err)
                    reject(err);
                else
    ...
    async createOAuthAccount(account) {
        await this.waitForReady();
        const run = promisify(this.db.run.bind(this.db));
        await run(`INSERT INTO oauth_accounts (
    ...
    async getOAuthAccount(provider, providerId) {
        await this.waitForReady();
        const get = promisify(this.db.get.bind(this.db));
        const row = await get('SELECT * FROM oauth_accounts WHERE provider = ? AND provider_id = ?', [provider, providerId]);
        if (!row)
            return null;
        return {
            id: row.id,
    ...
    async updateOAuthTokens(accountId, tokens) {
        await this.waitForReady();
        const run = promisify(this.db.run.bind(this.db));
        ...
        await run(`UPDATE oauth_accounts SET ${updates.join(', ')} WHERE id = ?`, values);
    }
    async deleteOAuthAccount(userId, provider) {
        await this.waitForReady();
        const run = promisify(this.db.run.bind(this.db));
        await run('DELETE FROM oauth_accounts WHERE user_id = ? AND provider = ?', [userId, provider]);
    }
    async createOAuthState(state) {
        await this.waitForReady();
        const run = promisify(this.db.run.bind(this.db));
        await run(`INSERT INTO oauth_states (state, provider, redirect_url, created_at, expires_at)
       VALUES (?, ?, ?, ?, ?)`, [state.state, state.provider, state.redirectUrl, state.createdAt, state.expiresAt]);
    }
    ...
    async deleteSessionKeysByEmail(email) {
        await this.waitForReady();
        // Find user by username (email)
        const user = await this.getUserByUsername(email.toLowerCase());
        if (!user)
        ...
        const result = await new Promise((resolve, reject) => {
            this.db.run('DELETE FROM session_keys WHERE user_id = ?', [user.id], function (err) {
                if (err)
                    reject(err);
                else
                    resolve({ changes: this.changes });
    ...
    async cleanupExpiredRecoveryTokens() {
        await this.waitForReady();
        const run = promisify(this.db.run.bind(this.db));
        await run('DELETE FROM recovery_tokens WHERE expires_at < ? OR used = 1', [new Date().toISOString()]);
    }
    // One-time token operations
    async markTokenUsed(tokenHash, expiresAt) {
        await this.waitForReady();
        const run = promisify(this.db.run.bind(this.db));
        await run('INSERT INTO used_tokens (token_hash, expires_at) VALUES (?, ?)', [tokenHash, expiresAt.toISOString()]);
    }
    async isTokenUsed(tokenHash) {
        await this.waitForReady();
        const get = promisify(this.db.get.bind(this.db));
        const result = await get('SELECT * FROM used_tokens WHERE token_hash = ? AND expires_at > ?', [tokenHash, new Date().toISOString()]);
        return !!result;
    }
    // Multi-sig account operations
    async createMultiSigAccount(account) {
        await this.waitForReady();
        const run = promisify(this.db.run.bind(this.db));
        const get = promisify(this.db.get.bind(this.db));
        await run('INSERT INTO multi_sig_accounts (id, name, address, required_signatures, created_by) VALUES (?, ?, ?, ?, ?)', [account.id, account.name, account.address, account.requiredSignatures, account.createdBy]);
        const created = await get('SELECT * FROM multi_sig_accounts WHERE id = ?', [account.id]);
        return this.mapMultiSigAccount(created);
    }
    async getMultiSigAccount(id) {
        await this.waitForReady();
        const get = promisify(this.db.get.bind(this.db));
        const account = await get('SELECT * FROM multi_sig_accounts WHERE id = ?', [id]);
        return account ? this.mapMultiSigAccount(account) : null;
    }
    async getMultiSigAccountsByUser(userId) {
        await this.waitForReady();
        const all = promisify(this.db.all.bind(this.db));
        const accounts = await all(`SELECT DISTINCT a.* FROM multi_sig_accounts a
       INNER JOIN multi_sig_members m ON a.id = m.account_id
       WHERE m.user_id = ? AND m.status = 'active'
       ORDER BY a.created_at DESC`, [userId]);
        return accounts.map((a) => this.mapMultiSigAccount(a));
    }
    async updateMultiSigAccount(id, updates) {
        await this.waitForReady();
        const run = promisify(this.db.run.bind(this.db));
        const fields = [];
        const values = [];
    ...
    async addMultiSigMember(member) {
        ...
        const run = promisify(this.db.run.bind(this.db));
        const get = promisify(this.db.get.bind(this.db));
        await run('INSERT INTO multi_sig_members (id, account_id, user_id, email, role, status) VALUES (?, ?, ?, ?, ?, ?)', [member.id, member.accountId, member.userId, member.email, member.role, member.status]);
        const created = await get('SELECT * FROM multi_sig_members WHERE id = ?', [member.id]);
        return this.mapMultiSigMember(created);
    }
    async getMultiSigMembers(accountId) {
        await this.waitForReady();
        const all = promisify(this.db.all.bind(this.db));
        const members = await all('SELECT * FROM multi_sig_members WHERE account_id = ? ORDER BY added_at ASC', [accountId]);
        return members.map((m) => this.mapMultiSigMember(m));
    }
    async updateMultiSigMemberStatus(id, status) {
        await this.waitForReady();
        const run = promisify(this.db.run.bind(this.db));
    ...
    async getMultiSigTransactionsByAccount(accountId, status) {
        ...
        return txs.map((t) => this.mapMultiSigTransaction(t));
    }
    ...
    async expireOldMultiSigTransactions() {
        await this.waitForReady();
        const result = await new Promise((resolve, reject) => {
            this.db.run(`UPDATE multi_sig_transactions SET status = 'expired' WHERE status = 'pending' AND expires_at < ?`, [new Date().toISOString()], function (err) {
                if (err)
                    reject(err);
                else
                    resolve({ changes: this.changes });
    ...
    async addMultiSigSignature(sig) {
        await this.waitForReady();
        const run = promisify(this.db.run.bind(this.db));
        const get = promisify(this.db.get.bind(this.db));
        await run('INSERT INTO multi_sig_signatures (id, transaction_id, signer_id, signer_address, status) VALUES (?, ?, ?, ?, ?)', [sig.id, sig.transactionId, sig.signerId, sig.signerAddress, sig.status]);
        const created = await get('SELECT * FROM multi_sig_signatures WHERE id = ?', [sig.id]);
        return this.mapMultiSigSignature(created);
    }
    async getMultiSigSignatures(transactionId) {
        await this.waitForReady();
        const all = promisify(this.db.all.bind(this.db));
        const sigs = await all('SELECT * FROM multi_sig_signatures WHERE transaction_id = ? ORDER BY signed_at ASC', [transactionId]);
        return sigs.map((s) => this.mapMultiSigSignature(s));
    }
    async getApprovalCount(transactionId) {
        await this.waitForReady();
        const get = promisify(this.db.get.bind(this.db));
        const result = await get(`SELECT COUNT(*) as count FROM multi_sig_signatures WHERE transaction_id = ? AND status = 'approved'`, [transactionId]);
        return result?.count || 0;
    }
    // Mapper functions
    mapMultiSigAccount(row) {
        return {
            id: row.id,
            name: row.name,
            address: row.address,
    ...
    async saveTokenMetadata(metadata) {
        ...
        const now = new Date().toISOString();
        await run(`INSERT OR REPLACE INTO token_metadata 
