...
export class BridgeCompleterService {
     ...
     */
    async checkBridge(sourceTxHash) {
        const bridge = this.pendingBridges.get(sourceTxHash);
        if (!bridge)
            return;
        try {
            // Check attestation status
            const response = await fetch(`${ATTESTATION_API}/v2/messages/${bridge.sourceDomain}?transactionHash=${sourceTxHash}`);
            if (!response.ok) {
     ...
     */
    async manualComplete(sourceTxHash, sourceDomain) {
        try {
            // Fetch attestation
            const response = await fetch(`${ATTESTATION_API}/v2/messages/${sourceDomain}?transactionHash=${sourceTxHash}`);
            if (!response.ok) {
...
// Singleton instance
let bridgeCompleterInstance = null;
export function getBridgeCompleterService() {
    if (!bridgeCompleterInstance) {
        bridgeCompleterInstance = new BridgeCompleterService();
    }
    return bridgeCompleterInstance;
}
//# sourceMappingURL=bridgeCompleterService.js.map