...
class AIService {
    ...
    async parseIntent(message, conversationHistory) {
        try {
     ...
     */
    async parseWithFineTunedModel(message, conversationHistory) {
...
IMPORTANT: Use conversation history to understand context. If user refers to "aynı adrese" (same address), "ona" (to them), "oraya" (there), look at previous messages for the address/token/amount.`,
     ...
     */
    async parseWithOpenAI(message, conversationHistory) {
        if (!this.openai)
