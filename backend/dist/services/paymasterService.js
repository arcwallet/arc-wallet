...
class PaymasterService {
    ...
    constructor() {
        ...
        if (!this.paymasterAddress) {
            ...
            this.paymasterContract = null; // Will be checked in isEnabled()
            return;
        }
        // Initialize contract only if address is set
        this.paymasterContract = new ethers.Contract(this.paymasterAddress, this.paymasterABI, this.provider);
    }
    /**
     ...
     */
    async getPaymasterData(sender, estimatedCost) {
        if (!this.isEnabled()) {
            return null;
        }
        try {
     ...
     */
    async canSponsor(sender, estimatedCost) {
        if (!this.isEnabled()) {
            return false;
        }
        try {
