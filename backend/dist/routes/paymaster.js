...
router.put('/policies', async (req, res) => {
    try {
        ...
        if (rateLimit !== undefined) {
            policyEngine.setRateLimitPolicy(rateLimit.enabled, rateLimit.maxOpsPerHour);
        }
        // Update budget policy
        if (budget !== undefined) {
            policyEngine.setBudgetPolicy(budget.enabled, budget.maxCostPerOp ? BigInt(budget.maxCostPerOp) : undefined, budget.dailyBudget ? BigInt(budget.dailyBudget) : undefined);
        }
        res.json({
            success: true,
