...
function generateOtpEmailHtml(otpCode) {
    return `
...
export const createCircleOtpRouter = (config, db, sessionStore) => {
     ...
     */
    router.post('/api/circle/otp/request', rateLimitMiddleware('registration'), async (req, res) => {
     ...
     */
    router.post('/api/circle/otp/resend', rateLimitMiddleware('registration'), async (req, res) => {
        const email = sanitizeEmail(req.body?.email);
        if (!email) {
            return res.status(400).json({ success: false, error: 'Please provide a valid email address.' });
        }
     ...
     */
    router.post('/api/circle/otp/verify', rateLimitMiddleware('auth'), async (req, res) => {
        const email = sanitizeEmail(req.body?.email);
