...
export class Database {
  ...
  async getTreasuryPolicy(walletAddress: string): Promise<TreasuryPolicy | null> {
    await this.waitForReady();
    const get: any = promisify(this.db.get.bind(this.db));
  ...
  async createTreasuryPolicy(policy: TreasuryPolicy): Promise<TreasuryPolicy> {
    await this.waitForReady();
    const run: any = promisify(this.db.run.bind(this.db));
    const get: any = promisify(this.db.get.bind(this.db));
  ...
  async updateTreasuryPolicy(walletAddress: string, updates: Partial<TreasuryPolicy>): Promise<TreasuryPolicy> {
    await this.waitForReady();
    const run: any = promisify(this.db.run.bind(this.db));
    ...
    if (updates.singleTransactionLimit !== undefined) { fields.push('single_transaction_limit = ?'); values.push(updates.singleTransactionLimit); }
    if (updates.requireApprovalAbove !== undefined) { fields.push('require_approval_above = ?'); values.push(updates.requireApprovalAbove); }
    if (updates.requiredSignatures !== undefined) { fields.push('required_signatures = ?'); values.push(updates.requiredSignatures); }
    if (updates.allowedOperations !== undefined) { fields.push('allowed_operations = ?'); values.push(JSON.stringify(updates.allowedOperations)); }
    if (updates.cooldownPeriod !== undefined) { fields.push('cooldown_period = ?'); values.push(updates.cooldownPeriod); }
    if (updates.maxYieldAllocation !== undefined) { fields.push('max_yield_allocation = ?'); values.push(updates.maxYieldAllocation); }
    if (updates.emergencyPauseEnabled !== undefined) { fields.push('emergency_pause_enabled = ?'); values.push(updates.emergencyPauseEnabled ? 1 : 0); }
    if (updates.whitelistedAddresses !== undefined) { fields.push('whitelisted_addresses = ?'); values.push(JSON.stringify(updates.whitelistedAddresses)); }
  ...
  private mapTreasuryPolicy(row: any): TreasuryPolicy {
    return {
      id: row.id,
      walletAddress: row.wallet_address,
  ...
  async createTreasuryTransaction(tx: TreasuryTransaction): Promise<TreasuryTransaction> {
    await this.waitForReady();
    const run: any = promisify(this.db.run.bind(this.db));
    const get: any = promisify(this.db.get.bind(this.db));
  ...
  async getTreasuryTransaction(id: string): Promise<TreasuryTransaction | null> {
    await this.waitForReady();
    const get: any = promisify(this.db.get.bind(this.db));
  ...
  async getTreasuryTransactions(walletAddress: string, status?: string): Promise<TreasuryTransaction[]> {
    await this.waitForReady();
    const all: any = promisify(this.db.all.bind(this.db));
    ...
    return rows.map((row: any) => this.mapTreasuryTransaction(row));
  }

  async updateTreasuryTransaction(id: string, updates: Partial<TreasuryTransaction>): Promise<void> {
    await this.waitForReady();
    const run: any = promisify(this.db.run.bind(this.db));
    ...
    if (updates.currentSignatures !== undefined) { fields.push('current_signatures = ?'); values.push(updates.currentSignatures); }
  ...
  private mapTreasuryTransaction(row: any): TreasuryTransaction {
    return {
      id: row.id,
      walletAddress: row.wallet_address,
  ...
  async createTreasurySignature(sig: TreasurySignature): Promise<TreasurySignature> {
    await this.waitForReady();
    const run: any = promisify(this.db.run.bind(this.db));
    const get: any = promisify(this.db.get.bind(this.db));

    await run(
      `INSERT INTO treasury_signatures (
  ...
  async getTreasurySignatures(transactionId: string): Promise<TreasurySignature[]> {
    await this.waitForReady();
    const all: any = promisify(this.db.all.bind(this.db));

    const rows = await all(
      'SELECT * FROM treasury_signatures WHERE transaction_id = ? ORDER BY signed_at ASC',
      [transactionId]
    );
    return rows.map((row: any) => this.mapTreasurySignature(row));
  }

  private mapTreasurySignature(row: any): TreasurySignature {
    return {
      id: row.id,
      transactionId: row.transaction_id,
      signerId: row.signer_id,
  ...
  async createTreasuryAuditLog(log: Omit<TreasuryAuditLog, 'id'> & { id?: string }): Promise<void> {
    await this.waitForReady();
    const run: any = promisify(this.db.run.bind(this.db));
  ...
  async getTreasuryAuditLogs(walletAddress: string, limit: number = 100, offset: number = 0): Promise<TreasuryAuditLog[]> {
    await this.waitForReady();
    const all: any = promisify(this.db.all.bind(this.db));

    const rows = await all(
      'SELECT * FROM treasury_audit_logs WHERE wallet_address = ? ORDER BY timestamp DESC LIMIT ? OFFSET ?',
  ...
  async recordTreasurySpending(walletAddress: string, token: 'USDC' | 'USYC', amount: number): Promise<void> {
    await this.waitForReady();
    const run: any = promisify(this.db.run.bind(this.db));
    const get: any = promisify(this.db.get.bind(this.db));
    ...
    const existing = await get(
      'SELECT * FROM treasury_spending WHERE wallet_address = ? AND date = ?',
      [walletAddress, today]
    );

    if (existing) {
      await run(
        `UPDATE treasury_spending SET ${column} = ${column} + ? WHERE wallet_address = ? AND date = ?`,
        [amount, walletAddress, today]
      );
    } else {
      await run(
        `INSERT INTO treasury_spending (id, wallet_address, date, usdc_spent, usyc_spent) VALUES (?, ?, ?, ?, ?)`,
        [
          `spending_${Date.now()}`,
          walletAddress,
          today,
  ...
  async getTodaySpending(walletAddress: string): Promise<{ usdcSpent: number; usycSpent: number }> {
    await this.waitForReady();
    const get: any = promisify(this.db.get.bind(this.db));
