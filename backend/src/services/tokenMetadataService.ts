...
import { ERC20_ABI, TokenMetadata } from '../contracts/ERC20.js';
...
export class TokenMetadataService {
     ...
     */
    async fetchAndCacheMetadata(address: string): Promise<TokenMetadata> {
    ...
    formatTokenAmount(value: string, decimals: number): string {
        try {
            ...
            if (num === 0) return '0';
            if (num >= 1000000) return num.toFixed(2); // Millions
            if (num >= 1000) return num.toFixed(2); // Thousands
            if (num >= 1) return num.toFixed(4); // Regular amounts
            if (num >= 0.0001) return num.toFixed(6); // Small amounts
     ...
     */
    async getFormattedAmount(tokenAddress: string, rawValue: string): Promise<string> {
     ...
     */
    async prePopulateCommonTokens(tokens: { address: string; symbol: string; name: string; decimals: number }[]): Promise<void> {
     ...
     */
    async batchFetchMetadata(addresses: string[]): Promise<Map<string, TokenMetadata | null>> {
