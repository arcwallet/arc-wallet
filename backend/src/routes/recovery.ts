...
export function createRecoveryRoutes(
  db: Database,
  config: EnvConfig,
  sessionStore?: MagicSessionStore
): Router {
  const router = Router();
   ...
   */
  router.post(
    '/backup-keys/add/calldata',
    authMiddleware(config.JWT_SECRET, sessionStore),
    rateLimitMiddleware('general'),
    validateRequestBody(['x', 'y', 'deviceName']),
    async (req: Request, res: Response, next: NextFunction) => {
      try {
   ...
   */
  router.post(
    '/backup-keys/remove/calldata',
    authMiddleware(config.JWT_SECRET, sessionStore),
    rateLimitMiddleware('general'),
    validateRequestBody(['x', 'y']),
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const { x, y } = req.body;
   ...
   */
  router.post(
    '/backup-keys/recover/calldata',
    rateLimitMiddleware('recovery'),
    validateRequestBody(['newPrimaryX', 'newPrimaryY', 'backupX', 'backupY', 'webAuthnSignature']),
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const { newPrimaryX, newPrimaryY, backupX, backupY, webAuthnSignature } = req.body;

        const calldata = recoveryService.buildRecoverWithBackupKeyCalldata(
          newPrimaryX,
          newPrimaryY,
          backupX,
          backupY,
   ...
   */
  router.post(
    '/guardian/add/calldata',
    authMiddleware(config.JWT_SECRET, sessionStore),
    rateLimitMiddleware('general'),
    validateRequestBody(['guardianAddress']),
    async (req: Request, res: Response, next: NextFunction) => {
      try {
   ...
   */
  router.post(
    '/guardian/threshold/calldata',
    authMiddleware(config.JWT_SECRET, sessionStore),
    rateLimitMiddleware('general'),
    validateRequestBody(['threshold']),
    async (req: Request, res: Response, next: NextFunction) => {
      try {
   ...
   */
  router.post(
    '/guardian/initiate/calldata',
    rateLimitMiddleware('recovery'),
    validateRequestBody(['newX', 'newY']),
    async (req: Request, res: Response, next: NextFunction) => {
      try {
  ...
  router.post(
    '/verify-backup-key',
    rateLimitMiddleware('general'),
    validateRequestBody(['walletAddress', 'x', 'y']),
    async (req: Request, res: Response, next: NextFunction) => {
      try {
  ...
  router.post(
    '/verify-guardian',
    rateLimitMiddleware('general'),
    validateRequestBody(['walletAddress', 'guardianAddress']),
    async (req: Request, res: Response, next: NextFunction) => {
      try {
