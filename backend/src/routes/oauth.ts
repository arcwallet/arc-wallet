...
export function createOAuthRouter(db: Database, config: EnvConfig, sessionStore: MagicSessionStore) {
    ...
    router.get('/:provider/callback', async (req, res) => {
        ...
        if (error) {
            return res.redirect(`${config.ORIGIN}/login?error=${encodeURIComponent(error as string)}`);
        }
