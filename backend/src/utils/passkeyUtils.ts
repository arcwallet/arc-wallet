        ...
        if (e.message && e.message.includes('Extra data')) {
            ...
            for (const len of commonLengths) {
                if (buffer.length > len) {
                    try {
                        const mainData = cbor.decode(buffer.slice(0, len)) as T;
 ...
 */
export function COSEECDHAtoXY(cosePublicKey: Uint8Array): [string, string] {
 ...
 */
export function COSEECDHAtoDER(cosePublicKey: Uint8Array): string {
 ...
 */
export function deriveAddressFromPublicKey(derPublicKey: string): string {
