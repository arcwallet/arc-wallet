...
export class TreasuryController {
  ...
  async getPolicy(req: Request, res: Response, next: NextFunction, authUserId?: string): Promise<void> {
    try {
  ...
  async createOrUpdatePolicy(req: Request, res: Response, next: NextFunction, authUserId?: string): Promise<void> {
    try {
      const { walletAddress } = req.params;
  ...
  async getTransactions(req: Request, res: Response, next: NextFunction, authUserId?: string): Promise<void> {
    try {
      ...
      const transactions = await this.db.getTreasuryTransactions(
        walletAddress,
  ...
  async signTransaction(req: Request, res: Response, next: NextFunction, authUserId?: string): Promise<void> {
    try {
      const { transactionId } = req.params;
      ...
      // Update transaction
      let newStatus: TreasuryTransaction['status'] = transaction.status;
  ...
  async executeTransaction(req: Request, res: Response, next: NextFunction, authUserId?: string): Promise<void> {
    try {
      const { transactionId } = req.params;
      ...
      // Record spending
      await this.db.recordTreasurySpending(
  ...
  async getAuditLogs(req: Request, res: Response, next: NextFunction, authUserId?: string): Promise<void> {
    try {
      const { walletAddress } = req.params;
      ...
      const logs = await this.db.getTreasuryAuditLogs(
        walletAddress,
  ...
  async getStatistics(req: Request, res: Response, next: NextFunction, authUserId?: string): Promise<void> {
    try {
      const { walletAddress } = req.params;
  ...
  private getDefaultPolicy(walletAddress: string): Omit<TreasuryPolicy, 'id' | 'createdAt' | 'updatedAt'> {
    return {
      walletAddress,
      dailyLimit: 100000,
  ...
  private async validateTransaction(
    ...
    amount: number,
    token: 'USDC' | 'USYC',
    userRole: TreasuryUserRole,
    usdcBalance: number,
    usycBalance: number
  ): Promise<{ allowed: boolean; requiresApproval: boolean; requiredSignatures: number; reason?: string }> {
    const policy = await this.db.getTreasuryPolicy(walletAddress) || this.getDefaultPolicy(walletAddress) as TreasuryPolicy;
  ...
  private canUserSign(role: TreasuryUserRole): boolean {
